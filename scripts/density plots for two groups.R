...
two_groups_plots <- function(df1, df2, df3) {
  ...
  plots <- lapply(names(df1), function(var) {
    ...
    plot <- ggplot(df3, aes_string(x = var, 
                                   fill = df3$a_f, 
                                   color = df3$a_f)) +
      geom_density(aes(y = after_stat(density)), adjust = 1.5, linewidth = 1) +
    ...
    plot <- plot + 
      ...
      annotate("text", x = Inf, y = Inf, hjust = 1, vjust = 4,
             ...
             size = 3.3, color = "#003399ff") + 
      annotate("text", x = Inf, y = 0, hjust = 1, vjust = 5.5,
             label = paste("Third Quartile =", round(surv_q3_value, 2)),
             size = 3.3, color = "#003399ff") + 
      annotate("text", x = Inf, y = 0, hjust = 2.6, vjust = 1,
      ...
      annotate("text", x = Inf, y = Inf, hjust = 1, vjust = 8,
             ...
             size = 3.3, color = "#CC9900FF") +
      annotate("text", x = Inf, y = Inf, hjust = 1, vjust = 9.5,
             label = paste("Mean =", round(non_mean_value, 2)),
             size = 3.3, color = "#CC9900FF") +
      annotate("text", x = Inf, y = Inf, hjust = 1, vjust = 11,
             label = paste("Median =", round(non_median_value, 2)),
             size = 3.3, color = "#CC9900FF") + 
      annotate("text", x = Inf, y = Inf, hjust = 1, vjust = 12.5,
  ...
  ggpubr::ggarrange(plotlist = plots[1:6], ncol = 2, nrow = 3, 
