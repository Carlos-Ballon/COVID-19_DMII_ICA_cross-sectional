...
group_stat_table_plot <- function(data_used, var, outcome) {
  ...
  stat_table <- data_used |>
    ...
    dplyr::rename("Shapiro-Wilk" = Shapiro_Wilk, 
  ...
  foot_note <- paste0("*IQR = Rango intercuartílico; ",
                      "Levene's test p-value = ", round(levene_p_value, 3), "; ",
                      "Bartlett’s test p-value = ", round(barlett_p_value, 3), ".\n",
  ...
  custom_stat_table <- stat_table |>
    ...
    ggpubr::tab_add_hline(at.row = 2:tab_nrow(stat_table), row.side = "bottom", linetype = 1) |>
    ggpubr::table_cell_bg(row = 2:tab_nrow(stat_table), column = 2, 
                          fill = "#CE3D32FF", color = "white", alpha = 0.3) |>
    ggpubr::table_cell_bg(row = 2:tab_nrow(stat_table), column = 4, 
                          fill = "#99CC00FF", color = "white", alpha = 0.3) |>
    ggpubr::table_cell_bg(row = 2, column = 1, 
                          fill = "#5773CCFF", color = "white", alpha = 0.3) |>
    ggpubr::table_cell_bg(row = 3, column = 1, 
                          fill = "#FFB900FF", color = "white", alpha = 0.3) |>
    ggpubr::tab_add_title(text = "Summary statistics and normality tests", hjust = -0.88,
                          padding = unit(1.5, "line"), size = 11, face = "italic") |>
    ggpubr::tab_add_footnote(text = foot_note, 
                             padding = unit(0.5, "line"), size = 10, face = "italic", hjust = 0.97)
  
  # Density plot by groups
  plot <- ggplot2::ggplot(data = data_used, 
                          aes_string(x = var, fill = outcome, color = outcome)) +
    ggplot2::geom_density(aes(y = after_stat(density)), adjust = 1.5, linewidth = 1, na.rm = TRUE) +
    ggplot2::labs(x = element_blank(), y = "Density", color = "Group", 
  ...
  ggpubr::ggarrange(plot, custom_stat_table, ncol = 1, heights = c(3, 1))
}
