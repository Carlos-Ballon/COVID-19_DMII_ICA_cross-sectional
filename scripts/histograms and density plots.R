...
total_plots <- function(.data) {
  ...
  plots <- lapply(names(.data), function(var) {
    ...
    plot <- ggplot(data = .data, aes_string(x = var)) +
      geom_histogram(aes(y = after_stat(density)), bins = 10, alpha = 0.8,
                     fill = "#3B3B3BFF", color = "white", na.rm = TRUE) +
      geom_density(fill = "#0073C2FF", col = "#00B5E2FF", 
                   alpha = 0.3, linewidth = 1, na.rm = TRUE) +
    ...
    plot <- plot + 
      annotate("text", x = Inf, y = 0, hjust = 1, vjust = 1.2,
